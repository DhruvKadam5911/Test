 ...
 *
 * server.js starts it for local development; api/index.js hands the same app
 * to Vercel's Node runtime, which invokes it per request and must never have
 * a port bound. Keep this file free of app.listen().
 */
...
// not need a code change — which matters because the allowlist is enforced in
// production and a missing entry blocks the app from its own API.
