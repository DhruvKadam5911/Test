...
`/music/*` reads from YouTube and nothing else. There is no `MUSIC_SOURCE`, no `PEERTUBE_HOST` and
no `YOUTUBE_STREAM_PROXY`: the second source, the selector that chose between them and the gate
that kept YouTube audio off a deployment were all removed together. See `services/youtube.js`.
