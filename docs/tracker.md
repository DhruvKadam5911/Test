...
| Date | Commit | Change |
|------|--------|--------|
| 2026-09-03 | — | **Mobile Player Controls Elevated & Sized For Easy One-Hand Reach.** Compacted the artwork poster on mobile to `min(200px, 50vw)` and added generous bottom safe-area padding (`max(32px, env(safe-area-inset-bottom) + 24px)`), bringing the track title, time scrubber, Play/Pause/Skip buttons, and Up Next bar higher up on the screen for comfortable one-handed mobile control in [MusicPage.jsx](file:///f:/Websites/Onion.tv/src/pages/MusicPage.jsx) |
| 2026-09-03 | — | **Player Controls Positioning & Stage View Layering Fixed.** Restored full visibility of desktop playback controls in Stage View (`zIndex: 70`), hid the Mobile Bottom Navigation Bar when Stage View is active to eliminate overlapping controls, raised Mobile Stage View to `zIndex: 65` with safe-area padding, and scaled mobile artwork to `min(240px, 60vw)` to lift transport controls (Play/Pause, Skip, Scrubber, Queue) into ergonomic thumb height in [MusicPage.jsx](file:///f:/Websites/Onion.tv/src/pages/MusicPage.jsx) |
