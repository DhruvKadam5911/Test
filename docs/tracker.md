...
| H3i | Pitch cannot be controlled while playback runs through the YouTube embed, and tempo only at the rates it advertises. Independent tempo and pitch needs the audio routed through Web Audio, which a cross-origin iframe cannot be — it needs audio the page serves itself |
| H3h | A YouTube video whose owner has turned off embedding still comes back from search, with artwork and a title, and shows "This video is unavailable" in the player. Labels and VEVO do it often — exactly the uploads the originality ranking puts first. `videos.list` now asks for `status` and drops them, and the player skips a track that errors with 101 or 150 |
